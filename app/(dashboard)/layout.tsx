...
export default function DashboardLayout({ children }: { children: ReactNode }) {
    ...
    // 3. Get the current route path to determine which link is active
        ...
        <div className="min-h-screen bg-slate-100 flex font-sans text-slate-900 overflow-hidden">
            ...
            >
                ...
                <div className="flex-1 overflow-y-auto px-4 py-2 space-y-1 custom-scrollbar">
                    {/* Main Links - 4. Add href and dynamic isActive check */}
                    <NavItem href="/overview" icon={HomeIcon} label="Dashboard" isActive={pathname === '/overview'} />
                    <NavItem href="/orders" icon={BagIcon} label="Orders" isActive={pathname === '/orders'} />
                    <NavItem href="/customers" icon={UsersIcon} label="Customers" isActive={pathname === '/customers'} />
                    <NavItem href="/messages" icon={ChatIcon} label="Messages" isActive={pathname === '/messages'} />

                    {/* Tools Section */}
                    <div className="pt-6 pb-2">
                        <p className="px-2 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Tools</p>
                    </div>
                    <NavItem href="/products" icon={TagIcon} label="Products" isActive={pathname === '/products'} />
                    <NavItem href="/integrations" icon={PlugIcon} label="Integrations" isActive={pathname === '/integrations'} />
                    <NavItem href="/analytics" icon={ChartIcon} label="Analytics" isActive={pathname === '/analytics'} />
                    <NavItem href="/invoices" icon={DocumentIcon} label="Invoice" isActive={pathname === '/invoices'} />
                    <NavItem href="/discounts" icon={PercentIcon} label="Discount" isActive={pathname === '/discounts'} />
                
                    {/* Settings Section */}
                    <div className="pt-6 pb-2">
                        <p className="px-2 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Settings</p>
                    </div>
                    <NavItem href="/settings" icon={CogIcon} label="Settings" isActive={pathname === '/settings'} />
                    <NavItem href="/security" icon={ShieldIcon} label="Security" isActive={pathname === '/security'} />
                    <NavItem href="/help" icon={HelpIcon} label="Help" isActive={pathname === '/help'} />
                </div>
            </aside>
...
// 5. Update NavItem interface to require `href`
function NavItem({ icon: Icon, label, href, isActive = false }: { icon: React.ElementType, label: string, href: string, isActive?: boolean }) {
    if (isActive) {
        return (
            // 6. Use <Link> instead of <a>
            <Link href={href} className="flex items-center gap-3 px-3 py-2.5 bg-violet-600 text-white rounded-xl text-sm font-medium shadow-md shadow-violet-200 mb-1">
    ...
    return (
        <Link href={href} className="flex items-center gap-3 px-3 py-2.5 text-slate-500 hover:text-slate-900 hover:bg-slate-50 rounded-xl text-sm font-medium transition-colors mb-1">
